...
public interface SurveyAnalysisRepository extends JpaRepository<Survey_analysis, Integer> {
    ...
    // 설문 응답자 연령별 비율
    @Query("select SAO from Survey_analysis SA join Survey_analysis_option SAO on SA.analysisId = SAO.analysisId where SA.analysisId = :analysisId and SA.surveyId = :surveyId")
